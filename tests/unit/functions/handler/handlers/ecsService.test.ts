...
describe('ECSServiceHandler', () => {
  ...
  describe('Teams notifications', () => {
    describe('start() with notifications', () => {
      it('should send Teams notification on successful start (Direct mode)', async () => {
        ...
        const handler = new ECSServiceHandler(sampleResource, configWithTeams);
        const result = await handler.start();
        ...
        expect(mockSendTeamsNotification).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            ...
            resourceType: 'ecs-service',
            resourceId: 'test-cluster/test-service',
            message: expect.stringContaining('Service started'),
          }),
          'test'
        );
      ...
      it('should send Teams notification on successful start (Auto Scaling mode)', async () => {
        const configWithTeamsAndAutoScaling: Config = {
          ...sampleConfig,
          notifications: {
            teams: {
              enabled: true,
        ...
        const handler = new ECSServiceHandler(sampleResource, configWithTeamsAndAutoScaling);
        const result = await handler.start();

        expect(result.success).toBe(true);
        expect(mockSendTeamsNotification).toHaveBeenCalledWith(
          configWithTeamsAndAutoScaling.notifications!.teams,
          expect.objectContaining({
            success: true,
            action: 'start',
            message: expect.stringContaining('Service started via Auto Scaling'),
          }),
          'test'
        );
      });
      ...
      it('should not send notification when Teams is disabled', async () => {
        ...
        const handler = new ECSServiceHandler(sampleResource, configWithDisabledTeams);
        await handler.start();
    ...
    describe('stop() with notifications', () => {
      it('should send Teams notification on successful stop (Direct mode)', async () => {
        const configWithTeams: Config = {
          ...sampleConfig,
          notifications: {
            teams: {
        ...
        expect(mockSendTeamsNotification).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            ...
            action: 'stop',
            resourceType: 'ecs-service',
            message: expect.stringContaining('Service stopped'),
          }),
          'test'
        );
      });

      it('should send Teams notification on successful stop (Auto Scaling mode)', async () => {
        const configWithTeamsAndAutoScaling: Config = {
          ...sampleConfig,
          notifications: {
            teams: {
        ...
        expect(mockSendTeamsNotification).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            success: true,
            action: 'stop',
            message: expect.stringContaining('Service stopped via Auto Scaling'),
          }),
          'test'
        );
      });
      ...
      it('should send Teams notification on failed stop', async () => {
        ...
        ecsMock.on(DescribeServicesCommand).rejects(new Error('Network timeout'));
