 ...
 *
 * Note: group_schedules uses human-readable format (startTime, stopTime, activeDays, enabled)
 * which is converted to group_schedules_cron by generate-cron.js at deploy time.
 * Lambda does not use group_schedules at runtime (only region_groups is used),
 * so we use passthrough validation to accept any format.
 */
const GroupSchedulesSchema = z.record(z.string(), z.object({}).passthrough());
...
const ConfigSchema = z
  .object({
    version: z.string().optional(), // Not used at runtime, only for documentation
    ...
    group_schedules: GroupSchedulesSchema.optional(), // Human-readable format, not used at runtime
    group_schedules_cron: z.record(z.string(), z.object({}).passthrough()).optional(), // Generated by generate-cron.js, not used at runtime
    discovery: z
      .object({
        method: z.string(),
